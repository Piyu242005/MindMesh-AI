...
def main():
    ...
    .stApp { background: var(--mm-black); color: var(--mm-white); }
    [data-testid="stHeader"] { background: var(--mm-black); }
    [data-testid="stSidebar"] { background: var(--mm-panel); border-right: 1px solid var(--mm-border); }
    [data-testid="stSidebar"] * { color: var(--mm-white); }
    h1, h2, h3, h4, p, label, span { color: var(--mm-white); }
    .stCaption, [data-testid="stCaptionContainer"] { color: var(--mm-muted) !important; }
    .stTabs [data-baseweb="tab-list"] { gap: 8px; border-bottom: 1px solid var(--mm-border); }
    .stTabs [data-baseweb="tab"] { color: var(--mm-muted); background: transparent; }
    .stTabs [aria-selected="true"] { color: var(--mm-red-bright) !important; border-bottom-color: var(--mm-red) !important; }
    .stButton > button { background: var(--mm-red); color: #fff; border: 1px solid var(--mm-red); border-radius: 8px; font-weight: 700; }
    .stButton > button:hover { background: var(--mm-red-bright); border-color: var(--mm-red-bright); color: #fff; }
    .stButton > button:disabled { background: #242424; color: #777; border-color: #333; }
    [data-testid="stFileUploader"] { background: var(--mm-panel-2); border: 1px dashed var(--mm-red); border-radius: 10px; }
    [data-baseweb="select"] > div, [data-baseweb="input"] > div, textarea, input { background: var(--mm-panel-2) !important; color: var(--mm-white) !important; border-color: var(--mm-border) !important; }
    [data-testid="stMetric"] { background: var(--mm-panel); border: 1px solid var(--mm-border); border-left: 3px solid var(--mm-red); padding: 12px; border-radius: 8px; }
    [data-testid="stMetricValue"] { color: var(--mm-red-bright) !important; }
    [data-testid="stChatMessage"] { background: var(--mm-panel); border: 1px solid var(--mm-border); border-radius: 10px; margin-bottom: 10px; }
    [data-testid="stChatInput"] textarea { background: var(--mm-panel-2) !important; border: 1px solid var(--mm-red) !important; }
    [data-testid="stExpander"] { background: var(--mm-panel); border: 1px solid var(--mm-border); border-radius: 8px; }
    .stDataFrame { border: 1px solid var(--mm-border); }
    hr { border-color: var(--mm-border); }
    a { color: var(--mm-red-bright) !important; }
    </style>
        ...
        if upload and st.button("⚡ Process & Index", disabled=not connected, use_container_width=True):
            try:
                with st.spinner("Processing and indexing…"):
                    text, kind = extract(upload)
                    count = index_document(upload.name, kind, text)
                st.success(f"Indexed {count} chunks")
            except Exception as e:
                st.error(f"Processing failed: {e}")
        st.divider()
        st.write("Database: `Supabase`" if connected else "Database: `Not configured`")
            ...
            with st.chat_message("assistant"):
                try:
                    with st.spinner("Searching knowledge…"):
                        ctx = search(q)
                        answer = llm(q, ctx, st.session_state.messages[:-1])
                    st.markdown(answer)
                    if ctx:
                        with st.expander("Sources"):
                            for x in ctx:
                                st.caption(f"{x.get('document')} · chunk {x.get('chunk_index')} · {float(x.get('similarity', 0)):.3f}")
    ...
    with analytics:
        cols = st.columns(4)
        cols[0].metric("Supabase", "Ready" if connected else "Missing")
        cols[1].metric("Embeddings", "Ready")
        cols[2].metric("LLM", "Ready" if secret("GROQ_API_KEY") or secret("GEMINI_API_KEY") else "Missing")
        cols[3].metric("Media", "FFmpeg")
