...
def get_qdrant():
    ...
    url, api_key = secret("QDRANT_URL"), secret("QDRANT_API_KEY")
...
def index_document(name: str, text: str) -> int:
    ...
    vectors = get_embedder().encode(chunks, normalize_embeddings=True).tolist()
    doc_id = hashlib.sha256(f"{name}:{text[:1000]}".encode()).hexdigest()[:16]
    points = []
    for i, (chunk, vector) in enumerate(zip(chunks, vectors)):
        # Qdrant accepts unsigned integer IDs; keep them deterministic per document/chunk.
        point_id = int(hashlib.sha256(f"{doc_id}:{i}".encode()).hexdigest()[:15], 16)
        points.append(PointStruct(id=point_id, vector=vector, payload={"document": name, "document_id": doc_id, "chunk": i, "text": chunk}))
...
def call_llm(question: str, context: Iterable[dict], history: list[dict]) -> str:
    context_text = "\n\n".join(f"[{x.get('document', 'source')} · chunk {x.get('chunk', '?')}]\n{x['text']}" for x in context)
    system = "You are MindMesh AI, a grounded knowledge assistant. Answer from supplied context when possible. If insufficient, say so. Cite sources as document name and chunk number. Keep answers concise."
    ...
    if groq_key:
        response = requests.post("https://api.groq.com/openai/v1/chat/completions", headers={"Authorization": f"Bearer {groq_key}"}, json={"model": secret("GROQ_MODEL", "llama-3.3-70b-versatile"), "messages": messages, "temperature": 0.2}, timeout=90)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        llm = "Groq" if secret("GROQ_API_KEY") else "Gemini" if secret("GEMINI_API_KEY") else "Not configured"
        st.write(f"LLM: `{llm}`")
