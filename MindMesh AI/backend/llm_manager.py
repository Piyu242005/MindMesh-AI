...
def _update_metrics(provider: str, model: str, duration: float, tokens: int = 0):
    """Update global or session metrics for Dashboard display."""
    try:
        # Streamlit context
        import streamlit as st
        ...
        st.session_state.llm_metrics["total_requests"] += 1
        st.session_state.llm_metrics["total_tokens"] += tokens
        st.session_state.llm_metrics["response_times"].append(duration)
        
        # Update original analytics tracking
        if "response_times" in st.session_state:
            st.session_state.response_times.append(round(duration, 2))
